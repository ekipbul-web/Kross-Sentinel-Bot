    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            await member.send(embed=discord.Embed(
                ...
                color=0xFFD700
            ).add_field(name="📋 Öneri No", value=f"#{oneri_no}").add_field(name="📊 Durum", value="⏳ Beklemede").set_footer(text="Sonuç DM ile bildirilecektir."))
        except: pass
...
class OneriDegerlendir(discord.ui.View):
    ...
    async def onayla(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if kullanici_id:
            try:
                ...
                await user.send(f"✅ **{interaction.guild.name}** | #{self.oneri_no} nolu öneriniz **ONAYLANDI!**")
            except: pass
    ...
    async def reddet(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if kullanici_id:
            try:
                user = await bot.fetch_user(int(kullanici_id))
                await user.send(f"❌ **{interaction.guild.name}** | #{self.oneri_no} nolu öneriniz **REDDEDİLDİ.**")
            except: pass
    ...
    async def ticket_create(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(
            ...
            description=f"**{member.mention}** bir destek talebi açtı!\n\n"
                       ...
                       f"**Kapatmak için** 🔒 butonuna tıkla.",
            color=0x00FF00,
            timestamp=datetime.now()
        )
