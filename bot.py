...
# Özel roller (her zaman yazabilir)
...
def ozel_rol_kontrol(member):
    """Kullanıcının özel rolleri var mı?"""
...
class OneriModal(discord.ui.Modal, title="💡 Öneri Formu"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        veri = veri_yukle()
        oneri_no = veri.get("oneri_sayac", 0) + 1
        veri["oneri_sayac"] = oneri_no
        
        veri["oneriler"].append({
            "no": oneri_no, "kullanici_id": member.id, "kullanici_adi": member.name,
            "oneri": self.oneri.value, "tarih": datetime.now().isoformat(),
            "durum": "bekliyor", "yorum": None
        })
        ...
        embed.add_field(name="📅 Tarih", value=f"<t:{int(datetime.now().timestamp())}:F>", inline=False)
        embed.add_field(name="📊 Durum", value="⏳ Beklemede", inline=True)
        embed.set_thumbnail(url=member.display_avatar.url)
        embed.set_footer(text="Kross Öneri Sistemi")
        
        view = OneriDegerlendir(oneri_no)
        await log_kanal.send(embed=embed, view=view)
        
        try:
            await member.send(embed=discord.Embed(
                title="💡 Öneriniz Alındı!", description=f"**{guild.name}** sunucusuna öneriniz iletildi.", color=0xFFD700
            ).add_field(name="📋 Öneri No", value=f"#{oneri_no}").add_field(name="📊 Durum", value="⏳ Beklemede").set_footer(text="Sonuç DM ile bildirilecek"))
        except: pass
...
class OneriDegerlendir(discord.ui.View):
    ...
    async def onayla(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if kullanici_id:
            try:
                ...
                embed = discord.Embed(title="✅ Öneriniz Onaylandı!", description=f"**{interaction.guild.name}** | **#{self.oneri_no}** nolu öneriniz onaylandı!", color=0x00FF00)
                embed.add_field(name="🌟 Teşekkürler!", value="Sizin gibi düşünen üyelerimiz olduğu için çok şanslıyız!", inline=False)
                embed.add_field(name="💫 İyi Eğlenceler!", value="Kross Ailesi olarak keyifli vakitler dileriz!", inline=False)
    ...
    async def reddet(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if kullanici_id:
            try:
                user = await bot.fetch_user(int(kullanici_id))
                embed = discord.Embed(title="❌ Öneriniz Reddedildi", description=f"**{interaction.guild.name}** | **#{self.oneri_no}** nolu öneriniz şu an için uygun görülmedi.", color=0xFF0000)
                embed.add_field(name="💭 Üzülmeyin!", value="Başka önerilerinizi bekliyoruz!", inline=False)
                embed.add_field(name="💫 İyi Eğlenceler!", value="Kross Ailesi olarak keyifli vakitler dileriz!", inline=False)
                await user.send(embed=embed)
            except: pass
    ...
    async def yorum(self, interaction: discord.Interaction, button: discord.ui.Button):
        modal = OneriYorumModal(self.oneri_no)
        await interaction.response.send_modal(modal)
...
class TicketKontrol(discord.ui.View):
    ...
    async def ticket_create(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        kanal = await guild.create_text_channel(
            ...
            category=kategori,
            topic=f"Ticket sahibi: {member.id} | Açılış: {datetime.now().strftime('%d.%m.%Y %H:%M')} | Alan: Yok",
        ...
        veri["tickets"][str(kanal.id)] = {
            "sahip": member.id, "alan": None, "acilis": datetime.now().isoformat(), "durum": "açık"
        }
...
class TicketYonetim(discord.ui.View):
    ...
    @discord.ui.button(label="🤝 Ticketi Al", style=discord.ButtonStyle.blurple, custom_id="ticket_al", emoji="🤝")
    async def ticket_al(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if ticket["alan"] is not None:
            return await interaction.response.send_message(f"❌ Bu ticket zaten <@{ticket['alan']}> tarafından alınmış!", ephemeral=True)
        
        # Destek yetkilisi veya özel rol kontrolü
        ...
        if not is_yetkili and not is_ozel:
            return await interaction.response.send_message("❌ Bu ticketi almaya yetkin yok!", ephemeral=True)
        
        ticket["alan"] = interaction.user.id
        veri_kaydet(veri)
        
        # Ticketi almayan yetkililerin yazma iznini kapat
        if yetkili_rol:
        ...
        embed = discord.Embed(title="🤝 Ticket Alındı", description=f"**{interaction.user.mention}** bu ticketi aldı!", color=0x5865F2)
        embed.set_footer(text="Başka bir yetkili yazamaz • Vazgeçmek için ↩️ butonu")
        await interaction.response.send_message(embed=embed)
    
    @discord.ui.button(label="↩️ Vazgeç", style=discord.ButtonStyle.grey, custom_id="ticket_vazgec", emoji="↩️")
    async def ticket_vazgec(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if ticket["alan"] != interaction.user.id:
            return await interaction.response.send_message("❌ Bu ticketi sen almamışsın ki vazgeçesin!", ephemeral=True)
        
        ticket["alan"] = None
        veri_kaydet(veri)
        
        # Yetkililerin yazma iznini geri aç
        yetkili_rol = get(interaction.guild.roles, name=DESTEK_YETKILI)
        if yetkili_rol:
            for member in yetkili_rol.members:
        ...
        embed = discord.Embed(title="↩️ Vazgeçildi", description=f"**{interaction.user.mention}** ticketten vazgeçti. Başka bir yetkili alabilir!", color=0xFFA500)
        await interaction.response.send_message(embed=embed)
    ...
    async def ticket_close(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Sadece ticketi alan veya özel rol kapatabilir
        veri = veri_yukle()
        ticket = veri["tickets"].get(str(interaction.channel.id))
        is_ozel = ozel_rol_kontrol(interaction.user)
        
        if ticket and ticket["alan"] != interaction.user.id and not is_ozel:
            return await interaction.response.send_message("❌ Bu ticketi sen almadın! Sadece alan yetkili kapatabilir.", ephemeral=True)
...
async def ticket_ekle(ctx, member: discord.Member):
    ...
    # Sadece ticketi alan veya özel rol ekleme yapabilir
    veri = veri_yukle()
    ticket = veri["tickets"].get(str(ctx.channel.id))
    is_ozel = ozel_rol_kontrol(ctx.author)
    
    if ticket and ticket["alan"] != ctx.author.id and not is_ozel:
        return await ctx.send("❌ Bu ticketi sen almadın! Sadece alan yetkili ekleme yapabilir.")
...
async def ticket_cikar(ctx, member: discord.Member):
    ...
    if ticket and ticket["alan"] != ctx.author.id and not is_ozel:
        return await ctx.send("❌ Bu ticketi sen almadın!")
...
class BasvuruModal(discord.ui.Modal, title="📝 YETKİLİ BAŞVURU FORMU"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        veri["basvurular"].append({
            "no": basvuru_no, "kullanici_id": interaction.user.id,
            "isim": self.isim.value, "yas": self.yas.value,
            "tecrube": self.tecrube.value, "neden": self.neden.value,
            "sure": self.sure.value, "durum": "bekliyor", "tarih": datetime.now().isoformat()
        })
        ...
        await log_kanal.send(f"{yetkili_rol.mention if yetkili_rol else ''} Yeni başvuru!", embed=embed, view=view)
...
class BasvuruDegerlendir(discord.ui.View):
    ...
    async def yeniden_degerlendir(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(f"🔄 #{self.basvuru_no} yeniden değerlendirmeye alındı! Onay/Red butonları aktif.")
...
async def on_ready():
    ...
    print("✅ Ticket, Başvuru, Öneri, Oylama hazır!")
...
async def yardim(ctx):
    ...
    embed.add_field(name="📊 İstatistik", value="`.istatistik`", inline=False)
...
if __name__ == "__main__":
    ...
    print("📩 Ticket | 📝 Başvuru | 💡 Öneri | 🗳️ Oylama")
